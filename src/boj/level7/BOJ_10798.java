...
다음에 두 번째 글자들을 세로로 읽는다. 이런 식으로 왼쪽에서 오른쪽으로 한 자리씩 이동 하면서 동일한 자리의 글자들을 세로로 읽어 나간다. 위의 그림 1의 다섯 번째 자리를 보면 두 번째 줄의 다섯 번째 자리의 글자는 없다. 이런 경우처럼 세로로 읽을 때 해당 자리의 글자가 없으면, 읽지 않고 그 다음 글자를 계속 읽는다. 그림 1의 다섯 번째 자리를 세로로 읽으면 D1gk로 읽는다.
...
public class BOJ_10798 {
    public static void main(String[] args) throws IOException {
        ...
        bw.write(result(array, max));
        bw.flush();
        bw.close();
        br.close();
    }
    public static String result(String arr [][], int max){
        String result = "";
        for(int i = 0 ; i < max ; i++){
            for(int j = 0 ; j<5 ; j++){
                if(arr[j][i] != null){
